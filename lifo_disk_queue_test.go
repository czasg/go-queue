...
func TestNewLifoDiskQueue1(t *testing.T) {
	...
	file.Close()
	queue, err := NewLifoDiskQueue(file.Name())
	if err != nil {
		panic(err)
	}
	name := "TestNewLifoDiskQueue"
	{
...
func TestNewLifoDiskQueue2(t *testing.T) {
	file, err := ioutil.TempFile("", "")
	if err != nil {
		panic(err)
	}
